...
<head>
    ...
    <title>@yield('title', 'SIM (Sistem Informasi Magang) Dispussipda Malang')</title>
...
<body class="bg-gray-50 text-gray-900 font-sans antialiased">
    ...
    : (auth()->user()
        ...
        : route('login')) }}" class="flex items-center gap-3">
                    <img src="{{ asset('img/logo_instansi.png') }}" alt="Logo Instansi"
                    ...
                    <ul class="hidden md:flex gap-8 text-sm font-medium text-gray-600">
                        @if(auth()->user()->isAdmin())
                            <li><a href="{{ route('admin.dashboard') }}"
                                    class="hover:text-blue-600 transition {{ request()->routeIs('admin.dashboard') ? 'text-blue-600' : '' }}">Dashboard</a>
                            </li>
                            <li><a href="{{ route('admin.mahasiswa.index') }}"
                                    class="hover:text-blue-600 transition {{ request()->routeIs('admin.mahasiswa.*') ? 'text-blue-600' : '' }}">Peserta</a>
                            </li>
                            <li><a href="{{ route('admin.groups.index') }}"
                                    class="hover:text-blue-600 transition {{ request()->routeIs('admin.groups.*') ? 'text-blue-600' : '' }}">Kelompok</a>
                            </li>
                            <li><a href="{{ route('admin.schedules.index') }}"
                                    class="hover:text-blue-600 transition {{ request()->routeIs('admin.schedules.*') ? 'text-blue-600' : '' }}">Jadwal</a>
                            </li>
                            <li><a href="{{ route('admin.locations.index') }}"
                                    class="hover:text-blue-600 transition {{ request()->routeIs('admin.locations.*') ? 'text-blue-600' : '' }}">Lokasi</a>
                            </li>
                            <li><a href="{{ route('admin.attendance.index') }}"
                                    class="hover:text-blue-600 transition {{ request()->routeIs('admin.attendance.*') ? 'text-blue-600' : '' }}">Presensi</a>
                            </li>
                            <li><a href="{{ route('admin.logbooks.index') }}"
                        ...
                        @else
                            <li><a href="{{ route('mahasiswa.dashboard') }}"
                                    class="hover:text-blue-600 transition {{ request()->routeIs('mahasiswa.dashboard') ? 'text-blue-600' : '' }}">Dashboard</a>
                            </li>
                            <li><a href="{{ route('mahasiswa.attendance.index') }}"
                                    class="hover:text-blue-600 transition {{ request()->routeIs('mahasiswa.attendance.*') ? 'text-blue-600' : '' }}">Presensi</a>
                            </li>
                            <li><a href="{{ route('mahasiswa.logbooks.index') }}"
                                    class="hover:text-blue-600 transition {{ request()->routeIs('mahasiswa.logbooks.*') ? 'text-blue-600' : '' }}">Logbook</a>
                            </li>
                            <li><a href="{{ route('mahasiswa.schedules.index') }}"
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @if(session('success'))
            <div class="mb-6 flex items-center gap-3 rounded-lg bg-green-50 border border-green-200 text-green-700 px-4 py-3 shadow-sm"
                ...
                <span class="flex-grow font-medium">{{ session('success') }}</span>
        ...
        @if(session('error'))
            <div class="mb-6 flex items-center gap-3 rounded-lg bg-red-50 border border-red-200 text-red-700 px-4 py-3 shadow-sm"
                ...
                <span class="flex-grow font-medium">{{ session('error') }}</span>
    ...
    <footer class="bg-white border-t border-gray-200 py-8 mt-auto">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <p class="text-sm text-gray-500">
                &copy; {{ date('Y') }} SIM (Sistem Informasi Magang) Dispussipda Malang
            </p>
        </div>
    </footer>
