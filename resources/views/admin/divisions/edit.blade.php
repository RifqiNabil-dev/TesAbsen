    ...
    <div class="p-6">
        <form method="POST" action="{{ route('admin.divisions.update', $division) }}" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $division->name) }}" class="w-full px-4 py-2 rounded border border-gray-300" required>
                @error('name')
            ...
            <div>
                ...
                <textarea name="description" class="w-full px-4 py-2 rounded border border-gray-300">{{ old('description', $division->description) }}</textarea>
                @error('description')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
